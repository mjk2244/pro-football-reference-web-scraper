...
def get_player_game_log(player = "Tom Brady", season = 2022) -> pd.DataFrame:
    ...
    if "QB" in position: return qb_game_log(soup)
    elif "WR" in position or "TE" in position: return wr_game_log(soup)
    elif "RB" in position: return rb_game_log(soup)
...
def make_request(player = "Tom Brady", season = 2022):
    ...
    # A potential solution is to go to https://www.pro-football-reference.com/players/A/ and retrieve the href for the correct Josh Allen, which may require the player's position
    # Similar issues exist for numerous other players, e.g. Damien Harris: https://www.pro-football-reference.com/players/H/HarrDa06/gamelog/2022/advanced/
    ...
    url = "https://pro-football-reference.com/players/%s/%s%s00/gamelog/%s/" % (last_initial, lname_first_four, fname_first_two, str(season))
...
def qb_game_log(soup: BeautifulSoup) -> pd.DataFrame:
    ...
    data = {"Date": [], "Week": [], "Team": [], "Game_Location": [], "Opp": [], "Result":[], "Cmp": [], "Att": [], "Pass_Yds": [], "Pass_TD": [], "Int": [], "Rating": [], "Sacked": [], "Rush_Att": [], "Rush_Yds": [], "Rush_TD": []}

    table_rows = soup.find('tbody').find_all('tr')
    # adding data to data dictionary
    for row in table_rows:
        data["Date"].append(row.find('td', {'data-stat': 'game_date'}).text)
        data["Week"].append(row.find('td', {'data-stat': 'week_num'}).text)
        ...
        data["Cmp"].append(row.find('td', {'data-stat': 'pass_cmp'}).text)
        data["Att"].append(row.find('td', {'data-stat': 'pass_att'}).text)
        data["Pass_Yds"].append(row.find('td', {'data-stat': 'pass_yds'}).text)
        data["Pass_TD"].append(row.find('td', {'data-stat': 'pass_td'}).text)
        data["Int"].append(row.find('td', {'data-stat': 'pass_int'}).text)
        data["Rating"].append(row.find('td', {'data-stat': 'pass_rating'}).text)
        data["Sacked"].append(row.find('td', {'data-stat': 'pass_sacked'}).text)
        data["Rush_Att"].append(row.find('td', {'data-stat': 'rush_att'}).text)
        data["Rush_Yds"].append(row.find('td', {'data-stat': 'rush_yds'}).text)
        data["Rush_TD"].append(row.find('td', {'data-stat': 'rush_td'}).text)
...
def wr_game_log(soup: BeautifulSoup) -> pd.DataFrame:
    ...
    # Most relevant WR stats, in my opinion. Could adjust if necessary (maybe figure out how to incorporate rushing stats?)
    data = {"Date": [], "Week": [], "Team": [], "Game_Location": [], "Opp":[],"Result": [], "Tgt": [], "Rec": [], "Rec_Yds": [], "Rec_TD": [], "Snap_Pct": []}
    ...
    for row in table_rows:
        ...
        data["Opp"].append(row.find('td', {'data-stat': 'opp'}).text)
        data["Result"].append(row.find('td', {'data-stat': 'game_result'}).text)
        data["Tgt"].append(row.find('td', {'data-stat': 'targets'}).text)
        data["Rec"].append(row.find('td', {'data-stat': 'rec'}).text)
        data["Rec_Yds"].append(row.find('td', {'data-stat': 'rec_yds'}).text)
        data["Rec_TD"].append(row.find('td', {'data-stat': 'rec_td'}).text)
        data["Snap_Pct"].append(row.find('td', {'data-stat': 'off_pct'}).text)
...
def rb_game_log(soup: BeautifulSoup) -> pd.DataFrame:
    ...
    data = {"Date": [], "Week": [], "Team": [], "Game_Location": [], "Opp": [], "Result": [], "Rush_Att": [], "Rush_Yds": [], "Rush_TD": [], "Tgt": [], "Rec_Yds": [], "Rec_TD": []}
...
def main():
    get_player_game_log(player = "Saquon Barkley", season = 2018)
