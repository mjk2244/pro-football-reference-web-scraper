...
def splits_averages(game_log: pd.DataFrame, grouping: str) -> pd.DataFrame:
    ...
    if grouping == 'game_location':
        results = game_log.groupby([grouping, 'result']).size().to_frame('count')
    game_log = game_log.groupby(grouping).mean(numeric_only=True)
    if grouping == 'game_location':
        game_log.insert(0, 'wins', [results['count'][1], results['count'][3]])
        game_log.insert(1, 'losses', [results['count'][0], results['count'][2]])
...
def main():
    print(win_loss('San Francisco 49ers', 2022, True))
    print(home_road('San Francisco 49ers', 2022, True))
